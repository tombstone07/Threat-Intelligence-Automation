...
def scan_workbook(bad_ips):
    ...
    try:
        ...
        bad_ip_count = 0
        log = []  # Log to store bad IPs found

        # Iterate over all worksheets, skipping 'cover page'
        for sheet in workbook.Sheets:
            if sheet.Name.lower() == 'cover page':
                continue
            ...
            print(f"Scanning sheet: {sheet.Name} with {rows} rows x {cols} columns...")
            ...
            for row in tqdm(range(1, rows + 1)):
                for col in range(1, cols + 1):
                    ...
                    if value in bad_ips:
                        ...
                        bad_ip_count += 1
                        log.append(f"Bad IP found: {value} in sheet: {sheet.Name} at row {row}, column {col}")

        # Log the count of bad IPs
        log.append(f"Total number of bad IPs found: {bad_ip_count}")
        print(f"Scan complete. {bad_ip_count} bad IPs found.")

        # Save the analyzed workbook
        print(f"Saving analyzed workbook to {save_path}...")
        workbook.SaveAs(os.path.abspath(save_path), Password=password)
        workbook.Close()
        excel.Quit()

        # Print the log of bad IPs
        for entry in log:
            print(entry)
